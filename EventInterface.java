	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == save) {
			...
			try {
				...
				year -= 1900;
				month -= 1;
				if((startHour >= 24) ||(startHour < 0) || (startMin < 0) || (startMin > 59)){
					throw new Exception("The start time is not valid.");
				}
				if((endHour > 24) ||(endHour < 0) || (endMin < 0) || (endMin > 59)){
					throw new Exception("The end time is not valid.");
				}
				if((startHour == endHour) && (startMin == endMin)){
					throw new Exception("End time cannot be the same as start time.");
				}
				double start = startHour + (startMin/60.0);
				double end = endHour + (endMin/60.0);
				if(start > end){
					throw new Exception("End time cannot preceed the start time.");
				}
