...
public class CalendarInterface extends Applet implements ActionListener, KeyListener, MouseListener {
	...
	private void drawWeek(Graphics g) {
		...
		//ADDED:
		EventData[] events = ctrl.getAllEvents();
		//END ADDED

		//Draw day-specific text
		g.setColor(Color.WHITE);	//Text Color
		Date current = selected;	//Get Date
		while (current.getDay() != 0) current = getLast(current);	//Traverse to sunday
		for (int i = 0; i < 7; i++) {		//For each day
			String date = current.toString().substring(0, 10);	//Get beginning of date string
			date += current.toString().substring(23, 28);		//Add ending of the date string
			Rectangle2D textBlock = metrics.getStringBounds(date, g);	//get the size of the date string
			int textX = (getWidth() / 3) + 4 + (width * (i % 2));		//Get X Coordinate
			int textY = (getHeight() / 10) + 1 + (height * (i / 2));	//Get Y Coordinate
			if (i == 6) {						//If saturday, draw to lower half of friday
				textX += width;
				textY -= (height / 2);
			}
			g.setColor(Color.GREEN);
			textY += textBlock.getHeight();		//Increment to next Date
			g.drawString(date, textX, textY);	//Draw the text
			//Added:
			for(EventData e : events){
				String tempString = e.getStart().toString().substring(0,10);
				tempString += e.getStart().toString().substring(23,28);
				if(tempString.equals(date)){
					//Increment textY by text height
					textY += 15;
					g.setColor(Color.GREEN);
					//if((textY + textBlock.getHeight()) < lowerboundY){
					if(i%2 == 0){
						if((textY+15) > (getHeight() / 10 + 1 + (height * ((i+1)/2)))){
							g.drawString(e.getTitle(), textX, textY);
						}
					} else {
						if((textY+15) < (getHeight() / 10 + 1 + (height * ((i+1)/2)))){
							g.drawString(e.getTitle(), textX, textY);
			...
			g.setColor(Color.WHITE);	//Text Color
			//END ADD
			current = getNext(current);			//Increment to the next date
		}
	}
